...
* **Bokeh Server security features!** Native SSL termination, and the ability to protect your Bokeh app with a login. [**``#9139``**](https://github.com/bokeh/bokeh/issues/9139), [**``#9140``**](https://github.com/bokeh/bokeh/issues/9140)
* **Embedding Bokeh applications directly in Django!** No Tornado necessary! [**``#9009``**](https://github.com/bokeh/bokeh/issues/9009)
* **JupyterLab/Ipywidgets Integrations!** [**``#9137``**](https://github.com/bokeh/bokeh/issues/9137)
...
Bokeh passed quite a milestone in October: our [Github repo](https://github.com/bokeh/bokeh) showed that in a month, there were over one million Bokeh downloads! That’s great to see!
<center>
...
While you’re checking out that new search capability, take a look at our clean, fresh new theme for [the documentation site](docs.bokeh.org). This beautiful makeover was made possible by **Joris Van den Bossche**, who shared some work he’d done on theming docs for another NumFocus project.
...
With the new ``jupyter_bokeh`` extension, it is now possible to render Bokeh figures and
...
``push_notebook()`` or handle Jupyter comms manually.

##### Embedding Bokeh Applications in Django #####

It's now possible to embed Bokeh applications in Django, without need for a Django-Tornado
bridge. This is possible thanks to the channels library, which handles asynchronous/websocket
traffic between Bokeh server and web browsers. *Note that Django integration works only on
Python 3.6+.*
...
... and other changes previously outlined in our post, [Preview of 2.0 Changes](https://blog.bokeh.org/posts/preview-2-0).
